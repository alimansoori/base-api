...
//            'script_url' => $this->url->get($this->router->getRewriteUri()),
    ...
    protected function trim_file_name($file_path, $name, $size, $type, $error,
            $index, $content_range) {
        // ButtonRemove path information and dots around the filename, to prevent uploading
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                ...
                if (!$content_range && $this->options['discard_aborted_uploads']) {
                    ...
                    if($file->blob_id)
                    {
                        $img = ModelBlobs::findFirstById($file->blob_id);
                        $img->delete();
                    }
                    unlink($file_path);
        ...
        foreach ($file_names as $file_name) {
            ...
            if ($success) {
                ...
                if(is_string($file_name))
                {
                    ...
                    $blobId = $exp_name[0];
                    $blob = ModelBlobs::findFirstById($blobId);
                    if($blob)
                    {
                        $blob->delete();
                    }
