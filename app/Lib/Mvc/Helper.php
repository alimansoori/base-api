...
//        return 'qa-styles.css?' . QA_VERSION;
    ...
    {
        $class = 'qa-template-' . Base::ilya_html($this->template);
        $class .= empty($this->theme) ? '' : ' qa-theme-' . Base::ilya_html($this->theme);

        if (isset($this->content['categoryids'])) {
            foreach ($this->content['categoryids'] as $categoryid) {
                $class .= ' qa-manage-' . Base::ilya_html($categoryid);
            }
        }
    ...
    public function post_meta_when($when, $class)
    {
        $this->output_split(@$when, $class . '-when');
    }
    ...
    public function q_title_list($q_list, $attrs = null)
    {
        ...
        foreach ($q_list as $q) {
            $this->output(
                ...
                '<a href="' . qa_q_path_html($q['postid'], $q['title']) . '" ' . $attrs . '>' . Base::ilya_html($q['title']) . '</a>',
                '</li>'
            );
        }
        $this->output('</ul>');
