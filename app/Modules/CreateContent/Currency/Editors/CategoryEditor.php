...
class CategoryEditor extends Editor
{
    ...
    public function initAjax(): void
    {
        $this->ajaxCreate->setUrl(
            $this->url->get([
                'for' => 'api_create_currency_category__'. ModelLanguage::getCurrentLanguage()
            ])
        );

        $this->ajaxEdit->setUrl(
            $this->url->get([
                'for' => 'api_edit_currency_category__'. ModelLanguage::getCurrentLanguage()
            ])
        );

        $this->ajaxRemove->setUrl(
            $this->url->get([
                'for' => 'api_remove_currency_category__'. ModelLanguage::getCurrentLanguage()
            ])
        );
    }
}